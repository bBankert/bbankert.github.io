...
int main(int argc, char *argv[]){
    ...
    while(input[0] != 'q' && input[0] != 'Q'){
        ...
        printf("Please enter an input string in the format '(operation)(sign)number'");
        printf("\nres = %d > ", res);
        // Get input string
        gets(input);
        // Clear screen
...
int neg(int a){
    ...
    int neg = ~a;
    neg = _add(neg,1);
    return neg;   // Replace 0 with code
}
...
int sub(int a, int b){
    b = neg(b);
    int diff = _add(a,b);
    return diff;  // Replace 0 with code
}
...
int convert(char *input){
    ...
    if(input[1] == '-'){
     //if negative sign is 1, else sign is 0
     sign = 1;
    }
